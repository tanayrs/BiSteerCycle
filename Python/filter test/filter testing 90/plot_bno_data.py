...
def plot_from_csv(path,settle_time_kal, settle_time_sfa, step_time):
    ...
    if settle_time_kal != np.inf:
        xticks.append(settle_time_kal+step_time)
    if settle_time_sfa != np.inf:
        xticks.append(settle_time_sfa+step_time)
    xticks.append(0)
    plt.xticks(xticks)

# Plots three chosen Sensor value readings individually
def chosen_plots():
    loop_path = './Python/filter test/filter testing 90/SensorData/BNO_Test_Step_'
    for i in [7,13,17]:
        path = loop_path + str(i) + '.csv'
...
def find_overshoot(path):
    ...
    angle_range_sfa = abs(max_angle_sfa - settle_angle_sfa)
    angle_range_kal = abs(max_angle_kal - settle_angle_kal)

    print(f'{max_angle_sfa=}, {settle_angle_sfa=}, {angle_range_sfa=}')
    ## Finding Overshot of SFA and Kalman ##
    if max_angle_sfa - settle_angle_sfa > 0:
        ...
        os_per_sfa = 100*np.abs(df['SFA'].max() - settle_angle_sfa)/angle_range_sfa
        os_per_kal = 100*np.abs(df['Kalman'].max() - settle_angle_kal)/angle_range_kal 
    ...
    for _,row in df.iterrows():
        ...
        if step_time == np.inf and abs(max_angle_sfa-row['SFA']) > 40:
...
def plot_comparison(DATA_PATH):
    ...
    plt.grid()
    plt.show()
    # print(f'{np.mean(settle_times_kal)=}')
    # print(f'{np.mean(settle_times_sfa)=}')
    # print(f'{np.mean(os_percs_kal)=}')
    # print(f'{np.mean(os_percs_sfa)=}')

# Plots all Sensor value readings in a combined plot
def all_plots():
    loop_path = './Python/filter test/filter testing 90/SensorData/BNO_Test_Step_'
    for i in range(1,21):
        plt.subplot(4,5,i)
...
if __name__ == '__main__':
    ...
    # DATA_PATH = './Python/filter test/filter testing 90/SensorData'
